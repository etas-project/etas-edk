module edk.browser.package_smoke;
...
flow main(args: Array<string>) -> i32 ![EdkBrowser.create, EdkBrowserMock.session, Error<BrowserError>, Error<SelectorError>] {
    ...
    if !selector_ok(parsed_result) { return 1; }
    if !is_css_selector(parsed) { return 1; }
    if !is_valid_selector(parsed) { return 1; }
    if !selector_ok(text_selector_result) { return 1; }
    if !is_text_selector(text_selector) { return 1; }
    if selector_spec(text_selector).value != "Send" { return 1; }
    if !is_valid_selector(text_selector) { return 1; }
    if selector_ok(empty_text_selector) { return 1; }
    if selector_ok(newline_selector) { return 1; }
    if selector_ok(newline_text_selector) { return 1; }
    if selector_ok(empty_selector) { return 1; }
    if url_value(target).host != "example.com" { return 1; }
    if url_value(child).path != "/docs" { return 1; }
    if !url_ok(target_result) { return 1; }
    if !url_ok(child_result) { return 1; }
    if url_ok(invalid_scheme) { return 1; }
    if url_ok(bad_host) { return 1; }
    if url_ok(bad_backslash_host) { return 1; }
    if url_ok(bad_at_host) { return 1; }
    if url_ok(bad_double_dot_host) { return 1; }
    if url_ok(bad_leading_dot_host) { return 1; }
    if url_ok(bad_trailing_dot_host) { return 1; }
    if url_ok(bad_leading_hyphen_host) { return 1; }
    if url_ok(bad_underscore_host) { return 1; }
    if url_ok(bad_path) { return 1; }
    if url_ok(bad_backslash_path) { return 1; }
    if url_ok(bad_embedded_url_path) { return 1; }
    if url_ok(bad_space_path) { return 1; }
    if url_ok(bad_tab_path) { return 1; }
    if url_ok(bad_fragment_path) { return 1; }
    if profile.name != "default" { return 1; }
    if !is_valid_browser_profile(profile) { return 1; }
    if is_valid_browser_profile(bad_profile) { return 1; }
    if is_valid_browser_profile(bad_profile_path) { return 1; }
    if is_valid_browser_profile(bad_profile_tab) { return 1; }
    if !is_valid_origin("HTTPS://EXAMPLE.COM") { return 1; }
    if !is_valid_origin("http://example.com") { return 1; }
    if is_valid_origin("https://example.com/app") { return 1; }
    if is_valid_origin("https://example.com?x=1") { return 1; }
    if is_valid_origin("file://example.com") { return 1; }
    if is_valid_origin("https://user@example.com") { return 1; }
    if is_valid_origin("https://example..com") { return 1; }
    if is_valid_origin("https://-example.com") { return 1; }
    if default_nav.timeout_millis != 30000 { return 1; }
    if custom_nav.wait_until != "networkidle" { return 1; }
    if !is_valid_navigation_options(default_nav) { return 1; }
    if !is_valid_navigation_options(custom_nav) { return 1; }
    if is_valid_navigation_options(bad_timeout_nav) { return 1; }
    if is_valid_navigation_options(bad_wait_nav) { return 1; }
    if click.high_impact { return 1; }
    if !high_impact.high_impact { return 1; }
    if screenshot.media_type != "image/png" { return 1; }
    if !screenshot.full_page { return 1; }
    if !is_valid_screenshot_request(screenshot) { return 1; }
    if is_valid_screenshot_request(bad_screenshot_media) { return 1; }
    if !is_supported_screenshot_media_type("image/jpeg") { return 1; }
    if is_supported_screenshot_media_type("image/svg+xml") { return 1; }
    if screenshot_output.media_type != "image/webp" { return 1; }
    if !is_valid_screenshot_ref(screenshot_output) { return 1; }
    if is_valid_screenshot_ref(empty_screenshot_output) { return 1; }
    if is_valid_screenshot_ref(unsafe_screenshot_output) { return 1; }
    if summary.node_count != 1 { return 1; }
    if count_snapshot_nodes(body_snapshot) != 1 { return 1; }
    if !snapshot_contains_text(snapshot, "Send") { return 1; }
    if snapshot_contains_text(snapshot, "") { return 1; }
    return 0;
}
