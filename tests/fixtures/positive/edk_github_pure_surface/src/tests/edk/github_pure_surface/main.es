module tests.edk.github_pure_surface.main;
...
flow check_refs_and_drafts() -> i32 ![] {
    ...
    let draft = with_idempotency_key(with_label(issue_draft("Bug", "body"), "bug"), "fixture-key");
    ...
    if github_repo_value(repo).owner != "etas-lang" { return 0; }
    if rest_path_value(repo_path(repo)) != "repos/etas-lang/etas" { return 0; }
    if !repo_ok(repo_result) { return 0; }
    if repo_ok(bad_repo) { return 0; }
    if repo_ok(space_repo) { return 0; }
    if repo_ok(query_repo) { return 0; }
    if repo_ok(at_repo) { return 0; }
    if repo_ok(backslash_repo) { return 0; }
    if repo_ok(colon_repo) { return 0; }
    if repo_ok(tab_repo) { return 0; }
    if github_repo_value(enterprise).api_host != "github.example.com" { return 0; }
    if !repo_ok(enterprise_result) { return 0; }
    if repo_ok(bad_enterprise) { return 0; }
    if repo_ok(space_enterprise) { return 0; }
    if repo_ok(query_enterprise) { return 0; }
    if repo_ok(backslash_enterprise) { return 0; }
    if repo_ok(colon_enterprise) { return 0; }
    if repo_ok(at_enterprise) { return 0; }
    if repo_ok(double_dot_enterprise) { return 0; }
    if repo_ok(leading_dot_enterprise) { return 0; }
    if repo_ok(trailing_dot_enterprise) { return 0; }
    if repo_ok(leading_dash_enterprise) { return 0; }
    if repo_ok(trailing_dash_enterprise) { return 0; }
    if repo_ok(tab_enterprise) { return 0; }
    if draft.idempotency_key != "fixture-key" { return 0; }
    if count_labels(draft) != 1 { return 0; }
    if !is_valid_idempotency_key(draft.idempotency_key) { return 0; }
    if !is_valid_issue_draft(draft) { return 0; }
    if is_valid_issue_draft(bad_title) { return 0; }
    if is_valid_issue_draft(bad_label) { return 0; }
    if is_valid_issue_draft(tab_label) { return 0; }
    if is_valid_issue_draft(missing_key) { return 0; }
    if is_valid_issue_draft(space_key) { return 0; }
    if is_valid_issue_draft(path_key) { return 0; }
    if is_valid_issue_draft(tab_key) { return 0; }
    if is_valid_idempotency_key("bad key") { return 0; }
    if is_valid_idempotency_key("bad\tkey") { return 0; }
    if pull_request_value(pr).number != 7 { return 0; }
    if branch_ref_value(head).name != "feature" { return 0; }
    if !branch_ok(head_result) { return 0; }
    if !branch_ok(base_result) { return 0; }
    if !branch_ok(docs_branch_result) { return 0; }
    if !pr_ok(pr_result) { return 0; }
    if !pr_ok(branch_pr) { return 0; }
    if !is_safe_ref_name("feature/docs") { return 0; }
    if !is_safe_ref_name("feature/docs-v2") { return 0; }
    if pr_ok(bad_pr) { return 0; }
    if branch_ok(leading_slash_branch) { return 0; }
    if branch_ok(trailing_slash_branch) { return 0; }
    if branch_ok(empty_component_branch) { return 0; }
    if branch_ok(dot_component_branch) { return 0; }
    if branch_ok(lock_component_branch) { return 0; }
    if branch_ok(reflog_branch) { return 0; }
    if branch_ok(wildcard_branch) { return 0; }
    if branch_ok(revision_branch) { return 0; }
    if branch_ok(colon_branch) { return 0; }
    if branch_ok(tab_branch) { return 0; }
    if is_safe_ref_name("feature/name^1") { return 0; }
    if is_safe_ref_name("feature/[bad]") { return 0; }
    if is_safe_ref_name("feature\tbad") { return 0; }
    if review_path_value(review_comment_value(review).path) != "src/main.es" { return 0; }
    if !review_path_ok(review_path_result) { return 0; }
    if !review_ok(review_result) { return 0; }
    if review_ok(bad_review) { return 0; }
    if review_path_ok(trailing_dotdot_review_path) { return 0; }
    if review_path_ok(backslash_review_path) { return 0; }
    if review_path_ok(drive_review_path) { return 0; }
    if review_path_ok(tab_review_path) { return 0; }
    return 1;
}

flow check_rest_and_webhook() -> i32 ![] {
    let repo = must_repo(github_repo("etas-lang", "etas"));
    let draft = with_idempotency_key(with_label(issue_draft("Bug", "body"), "bug"), "fixture-rest-key");
    let head = must_branch(branch_ref("feature"));
    let base = must_branch(branch_ref("main"));
    let pr = must_pr(pull_request(repo, 7, head, base));
    ...
    let pr_read_query = must_query(pr_query(pr));
    ...
    if issue_request.method != "POST" { return 0; }
    if rest_path_value(issue_request.path) != "repos/etas-lang/etas/issues" { return 0; }
    if rest_path_value(comment_request.path) != "repos/etas-lang/etas/pulls/7/comments" { return 0; }
    if rest_path_value(pr_request.path) != "repos/etas-lang/etas/pulls/7" { return 0; }
    if query_request.method != "GET" { return 0; }
    if !is_valid_query(pr_read_query) { return 0; }
    if query_ok(bad_query) { return 0; }
    if !query_ok(source_query) { return 0; }
    if !rest_path_ok(source_path_result) { return 0; }
    if rest_path_ok(absolute_query_path) { return 0; }
    if rest_path_ok(trailing_dotdot_query_path) { return 0; }
    if rest_path_ok(backslash_query_path) { return 0; }
    if rest_path_ok(drive_query_path) { return 0; }
    if rest_path_ok(tab_query_path) { return 0; }
    if !is_safe_repo_path("src/main.es") { return 0; }
    if is_safe_repo_path("/secret") { return 0; }
    if is_safe_repo_path("src/..") { return 0; }
    if is_safe_repo_path("src\\secret") { return 0; }
    if is_safe_repo_path("C:secret") { return 0; }
    if is_safe_repo_path("src\tsecret") { return 0; }
    if !good_signature.ok { return 0; }
    if bad_signature.ok { return 0; }
    if empty_signature.ok { return 0; }
    if tab_signature.ok { return 0; }
    return 1;
}

flow check_mocks() -> i32 ![] {
    let repo = must_repo(github_repo("etas-lang", "etas"));
    ...
    if issue.number != 42 { return 0; }
    if comment.id != "c-1" { return 0; }
    if result.kind != "pull_request" { return 0; }
    if result.rate_limit.remaining != 4999 { return 0; }
    return 1;
}
...
flow check_auth() -> i32 ![] {
    ...
    if !token_ref_blocked(token_result) { return 0; }
    if !token_ref_blocked(default_token_result) { return 0; }
    if !token_ref_invalid(bad_token_key) { return 0; }
    if !token_ref_invalid(bad_token_account) { return 0; }
    return 1;
}

flow main(args: Array<string>) -> i32 ![Error<GitHubError>] {
    if check_refs_and_drafts() + check_rest_and_webhook() + check_mocks() + check_auth() == 4 {
        return 0;
    }
    return 1;
}
