module tests.edk.git_pure_surface.main;
...
flow check_diff_and_patch() -> i32 ![] {
    ...
    if !ok.ok { return 0; }
    if bad.ok { return 0; }
    if absolute_bad.ok { return 0; }
    if backslash_bad.ok { return 0; }
    if drive_bad.ok { return 0; }
    if tab_bad.ok { return 0; }
    if rename_bad.ok { return 0; }
    if !new_file.ok { return 0; }
    if !patch_path_token_escapes_repo("a/src/../secret") { return 0; }
    if !patch_path_token_escapes_repo("a/src\\secret") { return 0; }
    if !patch_path_token_escapes_repo("a/C:secret") { return 0; }
    if !patch_path_token_escapes_repo("a/src\tsecret") { return 0; }
    if patch_path_token_escapes_repo("a/src/..hidden") { return 0; }
    if patch_path_token_escapes_repo("/dev/null") { return 0; }
    if count_diff_files(parsed) != 1 { return 0; }
    if count_total_hunks(parsed) != 1 { return 0; }
    return 1;
}

flow check_status_and_mocks() -> i32 ![Error<GitRefError>] {
    ...
    let bad_remote = remote("origin", "../etas.git");
    ...
    if git_repo_value(repo).name != "main" { return 0; }
    if !repo_ok(repo_result) { return 0; }
    if repo_ok(bad_name_repo) { return 0; }
    if repo_ok(bad_repo) { return 0; }
    if repo_ok(backslash_repo) { return 0; }
    if repo_ok(drive_repo) { return 0; }
    if repo_ok(tab_repo) { return 0; }
    if !remote_ok(origin_result) { return 0; }
    if !remote_ok(ssh_origin_result) { return 0; }
    if !is_valid_remote_ref(origin) { return 0; }
    if !is_valid_remote_ref(ssh_origin) { return 0; }
    if remote_ok(bad_remote) { return 0; }
    if remote_ok(bad_remote_name) { return 0; }
    if remote_ok(bad_remote_url) { return 0; }
    if remote_ok(tab_remote_url) { return 0; }
    if remote_ok(credential_remote) { return 0; }
    if remote_ok(exact_scheme_remote) { return 0; }
    if remote_ok(empty_host_remote) { return 0; }
    if remote_ok(empty_ssh_remote) { return 0; }
    if remote_ok(parent_remote) { return 0; }
    if remote_ok(scp_parent_remote) { return 0; }
    if remote_ok(scp_absolute_remote) { return 0; }
    if remote_ok(scp_missing_path_remote) { return 0; }
    if !branch_ok(branch("main")) { return 0; }
    if branch_value(must_branch(branch("main"))).name != "main" { return 0; }
    if branch_ok(bad_branch) { return 0; }
    ...
    if branch_ok(at_ref_branch) { return 0; }
    ...
    if is_safe_ref_name("feature/main.") { return 0; }
    if is_safe_ref_name("feature/name~1") { return 0; }
    if is_safe_ref_name("feature/name^1") { return 0; }
    if is_safe_ref_name("feature/[bad]") { return 0; }
    if is_safe_ref_name("feature\tbad") { return 0; }
    if path_escapes_repo("src/main.es") { return 0; }
    if !path_escapes_repo("../secret") { return 0; }
    if !path_escapes_repo("src\\secret") { return 0; }
    if !path_escapes_repo("C:secret") { return 0; }
    if !path_escapes_repo("src\tsecret") { return 0; }
    if path_escapes_repo("..hidden/repo") { return 0; }
    if count_status_entries(status.entries) != 1 { return 0; }
    if !status_entry_ok(good_entry_result) { return 0; }
    if !is_valid_status_entry(good_entry) { return 0; }
    if status_entry_ok(bad_entry) { return 0; }
    if status_entry_ok(backslash_entry) { return 0; }
    if status_entry_ok(drive_entry) { return 0; }
    if status_entry_ok(tab_entry) { return 0; }
    if status_entry_ok(tab_state_entry) { return 0; }
    if status.clean { return 0; }
    if modified.clean { return 0; }
    if !clean.status.clean { return 0; }
    if read.diff.text != diff_text { return 0; }
    if receipt.change_kind != "patch" { return 0; }
    return 1;
}

flow check_commit_message() -> i32 ![] {
    let message = commit_message("Update EDK", "body");
    let invalid = commit_message("", "");
    let subject_newline = commit_message("Update\nEDK", "body");
    let body_cr = commit_message("Update EDK", "body\rbad");

    if !is_valid_commit_message(message) { return 0; }
    if is_valid_commit_message(invalid) { return 0; }
    if is_valid_commit_message(subject_newline) { return 0; }
    if is_valid_commit_message(body_cr) { return 0; }
    return 1;
}

flow main(args: Array<string>) -> i32 ![Error<GitRefError>] {
    if check_diff_and_patch() + check_status_and_mocks() + check_commit_message() == 3 {
        return 0;
    }
    return 1;
}
