...
run_file_contains "transport maps substrate failures to HttpError" \
    ...
    'stream_timeout_for_phase("TLS response read")' \
    'stream_cancelled_for_phase("TLS request write")' \
    'stream_closed_for_phase("TLS request flush")' \
    'stream_interrupted_for_phase("TLS response read")' \
    'stream_limit_for_phase("TLS response read")' \
    'stream_host_for_phase("TLS response read")' \
    'stream_timeout_for_phase("TCP response read")' \
    'stream_cancelled_for_phase("TCP request write")' \
    'stream_closed_for_phase("TCP request flush")' \
    'stream_interrupted_for_phase("TCP response read")' \
    'stream_limit_for_phase("TCP response read")' \
    'stream_host_for_phase("TCP response read")' \
...
run_file_contains "transport closes open streams before raising source-visible errors" \
    ...
    "finish raise_http_error(stream_timeout_for_phase" \
    "finish raise_http_error(stream_host_for_phase" \
    "finish raise_http_error(tls_transport_error"

run_file_contains "transport matches every source-visible stream error variant" \
    src/edk/http/transport.es \
    "Error<StreamError>.raise(TimedOut)" \
    "Error<StreamError>.raise(Cancelled)" \
    "Error<StreamError>.raise(Closed)" \
    "Error<StreamError>.raise(Interrupted)" \
    "Error<StreamError>.raise(LimitExceeded)" \
    "Error<StreamError>.raise(StreamHost(_))"

run_file_contains "successful connection-close response ignores close errors" \
    src/edk/http/transport.es \
    "let raw = read_until_limit" \
    "close_tls_best_effort(stream);" \
    "close_tcp_best_effort(stream);" \
...
run_repo_file_contains "loopback server fixture covers real HTTP success and error routes" \
    tests/std-requirements/http/loopback_server.py \
...
run_effects_contains "transport requests std substrate actions" \
    src/edk/http/transport.es \
    "flow close_tls_best_effort" \
    "flow close_tcp_best_effort" \
...
run_pkg_run_value "Phase 2 stream variants map to stable kinds and phase messages" \
    "$ROOT/positive/edk_http_error_mapping_contract" \
    0
