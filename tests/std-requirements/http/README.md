...
- `positive/edk_http_error_mapping_contract` executes Phase 2 stable mappings
  for timeout, cancellation, closure, interruption, limit, and host failures;
  verifier source assertions bind those mappings to TCP/TLS write, flush, and
  response-read handlers and require successful response close to be best-effort.
  Phase 2 is complete within the existing connection-close substrate boundary.
